...
public class AlgoHUIMiner {
	...
	Map<Integer, HashMap<String, Double>> large = new HashMap<Integer, HashMap<String, Double>>();
	List<String> keys = null;
	Map<String, Double> small = new HashMap<String, Double>();
	...
	public Map<String, Double> getSmall() {
		return small;
	}

	public void setSmall(Map<String, Double> small) {
		this.small = small;
	}

	public Map<Integer, HashMap<String, Double>> getLarge() {
		return large;
	}

	public void setLarge(Map<Integer, HashMap<String, Double>> large) {
		this.large = large;
	}
	...
	public void runAlgorithm(String input, String output, int minUtility)
		...
		for (Integer item : mapItemToTWU.keySet()) {
			...
			//System.out.println("mapItemToTWU"+item+":"+mapItemToTWU.get(item));
		...
		//System.out.println(" high utility:"+listOfUtilityLists);
		...
		try {
			...
			while ((thisLine = myInput.readLine()) != null) {
				...
				//System.out.println("high utility:"+revisedTransaction);
	 ...
	 * @throws IOException
	 */
	private void huiMiner(int[] prefix, UtilityList pUL, List<UtilityList> ULs,
			int minUtility) throws IOException {
		//System.out.println("ULSsize:"+ULs.size());
...
//			
			//System.out.println("x.sumiutils:"+X.sumIutils+"   minutility:" + minUtility);
			if (X.sumIutils > minUtility) {
				// save to file
				writeOut(prefix, X.item, X.sumIutils);
			} else {
				if (s) {
					writeSamllOut(prefix, X.item, X.sumIutils);
				}
			}
			...
			//System.out.println("x.sumiutils&R:"+X.sumIutils+X.sumRutils+"   minutility:" + minUtility);
	...
	//	buffer.append(utility);
		if (large.get(k_large) == null) {
			large.put(k_large, new HashMap<String, Double>());
		}
//	 	System.out.println("key before :"+key);
		key = sortKey(key);
		buffer.append(key+" "+utility*1.0);
		
//	 	System.out.println("key :"+key);
		large.get(k_large).put(key, utility * 1.0);
		// write to file
		writer.write(buffer.toString());
		writer.newLine();
	}

	private void writeSamllOut(int[] prefix, int item, int utility)
			throws IOException {
		// huiCount++; // increase the number of high utility itemsets found
		...
		if (keys.contains(key)) {
			key = sortKey(key);
			System.out.println("Small Key:"+key);
			small.put(key, utility * 1.0);
		}
	...
	public void printStats() {
		System.out
				.println("=============  HUI-MINER ALGORITHM - STATS =============");
