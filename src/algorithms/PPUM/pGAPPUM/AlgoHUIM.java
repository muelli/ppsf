...
public class AlgoHUIM {
	...
	List<Set<Integer>> HUISet=new ArrayList<Set<Integer>>();
	List<Set<Integer>> PrelagerSet=new ArrayList<Set<Integer>>();
	 ...
	 */
	public void runAlgorithm(String input, String output, int minUtility,int lowUtility) throws IOException {
		// reset maximum
		maxMemory =0;
		...
		huiMiner(new int[0], null, listOfUtilityLists, minUtility,lowUtility);
	 ...
	 * @throws IOException
	 */
	private void huiMiner(int [] prefix, UtilityList pUL, List<UtilityList> ULs, int minUtility,int lowUtility)
			throws IOException {
		...
		for(int i=0; i< ULs.size(); i++){
			...
			if(X.sumIutils >= minUtility){
				// save to file
				writeHUI(prefix, X.item, X.sumIutils);
			}
			else if(X.sumIutils >=lowUtility &&X.sumIutils < minUtility){
				writePrelager(prefix, X.item, X.sumIutils);
			}
			...
			if(X.sumIutils + X.sumRutils >= minUtility){
				...
				huiMiner(newPrefix, X, exULs, minUtility,lowUtility); 
			}
		}
	}
	...
	private void writeHUI(int[] prefix, int item, int utility){
		huiCount++; // increase the number of high utility itemsets found
		//Create a string buffer
		Set<Integer> Huitem=new TreeSet<Integer>();
		// append the prefix
		for (int i = 0; i < prefix.length; i++) {
			Huitem.add(prefix[i]);
		}
	...
	private void writePrelager(int[] prefix, int item, int utility){
		prelagerCount++; // increase the number of prelager high utility itemsets found
		//Create a string buffer
		Set<Integer> Huitem=new TreeSet<Integer>();
		// append the prefix
		for (int i = 0; i < prefix.length; i++) {
		...
		// append the last item
		Huitem.add(item);
		//System.out.println(Huitem+"============:"+utility);
		PrelagerSet.add(Huitem);
		// append the utility value
	}
	...
	public void printStats() {
		...
		System.out.println(" Minging Memory ~ " + maxMemory+ " MB");
		System.out.println(" High-utility itemsets count : " + huiCount);
		System.out.println(" Prelager-High-utility itemsets count : " + prelagerCount);
	...
	/*public static void main(String args[]) throws IOException{
		...
		int low_Utility=350000;
		String input="../HUIMiner/database/retail_utility.txt";
		String output="../HUIMiner/database/resultofretail.txt";
		ahuim.runAlgorithm(input, output, min_Utility,low_Utility);
		ahuim.printStats();
	}*/
}